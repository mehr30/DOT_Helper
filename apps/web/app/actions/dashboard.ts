...
export async function getDashboardStats(): Promise<DashboardStats> {
    ...
    const [driverCount, vehicleCount, activeAlerts, expiringDrivers, expiringVehicles] =
        await Promise.all([
            ...
            // CDL and medical card expirations within 60 days
            ...
            // Annual inspections due within 60 days
    ...
    for (const d of expiringDrivers) {
        if (d.cdlExpiration) {
            const cdlDays = Math.ceil(
                (d.cdlExpiration.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
            );
            if (cdlDays <= 60) {
                upcomingExpirations.push({
                    id: `cdl-${d.id}`,
                    title: `CDL Expiration — ${d.firstName} ${d.lastName}`,
                    date: d.cdlExpiration.toISOString(),
                    type: "driver",
                    daysLeft: cdlDays,
                });
            }
        ...
        if (d.medicalCardExpiration) {
            const medDays = Math.ceil(
                (d.medicalCardExpiration.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
            );
            if (medDays <= 60) {
                upcomingExpirations.push({
                    id: `med-${d.id}`,
                    title: `Medical Card — ${d.firstName} ${d.lastName}`,
                    date: d.medicalCardExpiration.toISOString(),
                    type: "driver",
                    daysLeft: medDays,
                });
            }
    ...
    for (const v of expiringVehicles) {
        if (v.annualInspectionDue) {
            const days = Math.ceil(
                (v.annualInspectionDue.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
            );
            upcomingExpirations.push({
                id: `insp-${v.id}`,
                title: `Annual Inspection — Unit ${v.unitNumber}`,
                date: v.annualInspectionDue.toISOString(),
                type: "vehicle",
                daysLeft: days,
            });
        }
    ...
    return { driverCount, vehicleCount, activeAlerts, upcomingExpirations };
}
