...
export default function Page() {
    return (
        <BlogPostLayout category="Driver Management" title="The FMCSA Clearinghouse: A Practical Guide for Employers" date="February 23, 2026" readTime="8 min read" relatedPosts={[{ slug: "driver-qualification-file-requirements", title: "Driver Qualification File Requirements" }, { slug: "drug-alcohol-testing-compliance", title: "Drug & Alcohol Testing Compliance" }]}>
            ...
            <h3>Full Query</h3>
            <p>Required at <strong>pre-employment</strong> for every new driver. Reveals all detailed drug &amp; alcohol violation records. Costs <strong>$1.25 per query</strong>. Requires <strong>electronic driver consent</strong> — the driver must log into the Clearinghouse and approve your query before you can see the results.</p>
            ...
            <h3>Limited Query</h3>
            <p>Required <strong>annually</strong> for all current drivers. Tells you only whether a violation exists — yes or no. Costs <strong>$1.25 per query</strong>. If the result comes back positive, you must follow up with a full query (which requires driver consent).</p>
            <p>You need a <strong>general consent form</strong> signed by the driver to run limited queries. This is a one-time consent, not per-query. Get it signed during onboarding and keep it in the DQF.</p>

            <h2>Reporting Requirements</h2>
            <p>Employers and MROs must report to the Clearinghouse:</p>
            <ul>
                <li>Positive drug test results (reported by MRO)</li>
                <li>Alcohol test results of 0.04+ BAC (reported by employer)</li>
                <li>Refusals to test (reported by employer)</li>
                <li>Actual knowledge violations (reported by employer)</li>
                <li>Return-to-duty and follow-up test results (reported by SAP/employer)</li>
            </ul>
